export default {
  ...
  identity: {
    listTitle: "身份管理",
    listHint: "玩家身份独立于 AI 角色，可拥有多套人设并在对话中切换扮演。",
    create: "新建身份",
    createTitle: "新建身份",
    editTitle: "编辑身份",
    edit: "编辑",
    delete: "删除",
    empty: "暂无身份（默认身份应始终存在）",
    noSubtitle: "未设置副标题",
    defaultBadge: "默认身份",
    possessedBadge: "当前扮演",
    possess: "扮演",
    ...
    fields: {
      ...
      subtitle: "副标题",
      subtitlePlaceholder: "例如：普通高中生",
      prompt: "人设提示词",
      promptPlaceholder: "描述这个身份的性格、背景与行为方式",
      info: "简介",
      infoPlaceholder: "可选：一句话介绍",
      speechExamples: "说话风格",
      speechExamplesPlaceholder: "可选：示例台词，用于让 AI 模仿语气",
    },
    actions: {
      save: "保存",
      saving: "保存中...",
    ...
    messages: {
      ...
      saveFailed: "保存身份失败",
      deleteFailed: "删除身份失败",
      createSuccessTitle: "身份创建成功",
      createSuccess: "身份「{name}」已创建",
      updateSuccessTitle: "身份更新成功",
      updateSuccess: "身份「{name}」已更新",
      deleteSuccessTitle: "身份已删除",
      deleteSuccess: "身份「{name}」已删除",
      deleteConfirmTitle: "删除身份",
      deleteConfirm: "确定删除身份「{name}」吗？其发言将归属默认身份，相关记忆会被清除。",
    },
  },
  sceneEdit: {
    title: {
  ...
  llmProviders: {
    ...
    form: {
      ...
      visionCompress: "识图时压缩图片",
      visionCompressHint: "默认关闭，图片原图直发。开启后可控制边长与质量，节省 token 与上下文缓存",
      visionMaxEdge: "最大边长（像素）",
      visionMaxEdgeHint: "超宽图片等比缩放到此边长（64–8192）",
      visionQuality: "编码质量（0-100）",
      visionQualityHint: "数值越高越清晰，体积（token 占用）也越大",
      save: "保存",
      cancel: "取消",
    },
    test: {
  ...
  memoryDebug: {
    title: "永久记忆调试",
    rolesTitle: "角色",
    noRoles: "当前没有可检视的角色（先在对话里让角色上场）",
    selectRoleHint: "选择左侧角色，查看其永久记忆与真实上下文",
    runtimeOn: "记忆已启用",
    runtimeOff: "记忆已关闭",
    runtimeAbsent: "运行时未创建",
    saveBound: "存档 #{id}",
    saveNone: "自由对话：记忆只存在内存里，重启即失",
    refresh: "刷新",
    refreshing: "刷新中…",
    triggerTitle: "压缩状态（运行时生效值）",
    triggerHint:
      "阈值单位是「该角色可见的非 system 台词」：别的角色的台词、System 行、该角色感知不到的台词都不计数。",
    triggerProgress: "已累积 {visible} / 阈值 {interval} 条",
    triggerRemaining: "距触发还差 {n} 条",
    triggerReached: "已达阈值",
    pointerOutOfRange: "压缩指针越界：下次触发时会被重置为 0（撤回 / 读档 / 清空都会让历史变短）",
    runtimeEffective: "以上 interval / window 是运行时生效值，改设置需重启才会更新。",
    storeTitle: "① 记忆库存储真源",
    storeHint: "完整、不截断",
    charsOf: "{stored} 字 / 上限 {limit} 字",
    charsNoLimit: "{stored} 字 / 不截断",
    truncatedTag: "注入时会被截断",
    metaLine: "压缩指针 {pointer} · 更新于 {updated} · schema v{schema}",
    section: {
      short_term: "近期回顾（短期摘要）",
      long_term: "长期经历",
      user_info: "ta 的信息",
      promises: "重要约定",
    },
    injectedTitle: "② 注入视图",
    injectedHint: "LLM 实际收到的内容，已按上限截断",
    injectedSystem: "system 记忆块（不含短期摘要）",
    injectedShortTerm: "短期回顾前缀（拼在第一条 user 前）",
    injectedShortTermEmpty: "未注入",
    contextTitle: "③ 真实上下文",
    contextHint: "共 {n} 条，下一次请求将以此为基础",
    contextEmpty: "上下文为空（该角色还没参与过台词）",
    toolCalls: "工具调用",
