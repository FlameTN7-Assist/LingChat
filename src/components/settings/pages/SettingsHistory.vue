<template>
  <MenuPage>
    <MenuItem :title="$t('settings.history.title')">
      ...
      <div class="flex h-full max-h-[75dvh] min-h-0 flex-col">
        <div v-if="dialogHistory.length === 0" class="flex flex-1 items-center justify-center">
          <div
            class="py-10 text-center text-2xl font-bold text-gray-100
        ...
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div
            ref="contentRef"
            class="min-h-0 flex-1 scrollbar-thin
              [scrollbar-color:var(--accent-color,#79d9ff)_transparent] overflow-y-auto
              scroll-smooth px-1.5 py-3.5"
              ...
              >
                ...
                <div v-if="item.thinking" class="mb-1">
                  <button
                    class="inline-flex cursor-pointer items-center gap-1 rounded-full border
                      border-[rgba(121,217,255,0.25)] bg-[rgba(121,217,255,0.08)] px-2.5 py-0.5
                      text-xs text-[#a8d8f0]/70 transition-all duration-200
                      hover:border-[rgba(121,217,255,0.5)] hover:text-[#c9e7ff]"
                    @click.stop="toggleThinking(i)"
                  >
                    <span>{{ isThinkingExpanded(i) ? "▼" : "▶" }}</span>
                    <span>{{
                      $t("settings.history.thinking", { count: item.thinking.length })
                    }}</span>
                  </button>
                  <div
                    v-if="isThinkingExpanded(i)"
                    class="mt-1.5 max-h-64 scrollbar-thin overflow-y-auto rounded-2xl border
                      border-[rgba(121,217,255,0.15)] bg-[rgba(121,217,255,0.05)] px-4 py-3
                      text-[15px] leading-normal whitespace-pre-wrap text-white/55"
                  >
                    {{ item.thinking }}
                  </div>
                </div>
                <template v-for="(entry, j) in item.lines" :key="j">
                  <div
                    ...
                    :key="k"
                    class="flex items-start gap-1.5 py-0.5 wrap-break-word whitespace-pre-wrap"
                    :class="{
                  ...
                  >
                    ...
                    <button
                      ...
                      class="mt-0.5 inline-flex h-5.5 w-5.5 shrink-0 cursor-pointer items-center
                        justify-center rounded border-0 bg-[rgba(121,217,255,0.15)]
                        text-(--accent-color,#79d9ff) transition-all duration-200
                        hover:bg-[rgba(121,217,255,0.35)] hover:text-white"
                    ...
                    <button
                      ...
                      class="mt-0.5 inline-flex h-5.5 w-5.5 shrink-0 cursor-pointer items-center
                        justify-center rounded border-0 bg-[rgba(121,217,255,0.1)] text-white/35
                        transition-all duration-200 hover:bg-[rgba(121,217,255,0.35)]
                        hover:text-white disabled:cursor-wait disabled:opacity-50"
                      :title="$t('settings.history.generateVoice')"
                      :disabled="isGeneratingVoice(entry)"
                      @click="generateVoice(entry)"
                    >
                      <LoaderCircle
                        v-if="isGeneratingVoice(entry)"
                        :size="16"
                        class="animate-spin"
                      />
          ...
          >
            <button
              class="cursor-pointer rounded-lg border-0 bg-[#e9ecef] px-4 py-1.5 text-sm font-medium
                text-[#495057] transition-all duration-200 hover:not-disabled:-translate-y-0.5
                hover:not-disabled:bg-(--accent-color,#79d9ff) hover:not-disabled:text-white
                hover:not-disabled:shadow-[0_4px_10px_rgba(121,217,255,0.4)]
  ...
  async function handleBacktrack(messageSeq: number) {
    ...
    try {
      ...
      await dialogStore.alert(
        t("settings.history.backtrackFailed", {
