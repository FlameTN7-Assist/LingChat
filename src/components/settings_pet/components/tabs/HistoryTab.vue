<template>
  <article class="flex h-full min-h-0 w-full flex-col">
    <!-- 头部区域 -->
    <header
    ...
    <div class="flex min-h-0 flex-1 flex-col gap-3">
      ...
      <div
        ...
        class="flex flex-1 flex-col items-center justify-center rounded-xl border-2 border-dashed
          p-8 transition-all"
        :class="
          isDarkMode
      ...
      <div v-else class="flex min-h-0 flex-1 flex-col gap-4">
        ...
        <div
          ref="contentRef"
          class="min-h-0 flex-1 overflow-y-auto scroll-smooth rounded-xl border p-4 shadow-sm
            transition-all"
        ...
        >
          <template v-for="(item, i) in groupedHistory" :key="i">
            <div
              class="py-1"
              :class="{ 'mt-0 border-t pt-3': !item.isNarration && i > 0 }"
              :style="
                isDarkMode
            ...
            >
              ...
              <div v-if="item.thinking" class="mb-1">
                <button
                  class="inline-flex cursor-pointer items-center gap-1 rounded-full border px-2.5
                    py-0.5 text-xs transition-all duration-200"
                  :class="
                    isDarkMode
                      ? `border-sky-400/25 bg-sky-400/10 text-sky-200/70 hover:border-sky-400/50
                        hover:text-sky-100`
                      : `border-sky-200 bg-sky-50 text-sky-500/80 hover:border-sky-300
                        hover:text-sky-600`
                  "
                ...
                <div
                  ...
                  class="mt-1.5 max-h-64 scrollbar-thin overflow-y-auto rounded-2xl border px-4 py-3
                ...
                >
                  ...
                  <button
                    ...
                    class="mt-0.5 inline-flex h-5.5 w-5.5 shrink-0 cursor-pointer items-center
                      justify-center rounded border-0 transition-all duration-200"
                    :class="
                      isDarkMode
                        ? `bg-[rgba(121,217,255,0.15)] text-sky-400
                  ...
                  <button
                    ...
                    :class="
                      isDarkMode
                        ? `bg-[rgba(121,217,255,0.1)] text-white/40
                          hover:bg-[rgba(121,217,255,0.35)] hover:text-white`
        ...
        <!-- 分页控制器 -->
        <div v-if="totalPages > 1" class="flex shrink-0 items-center justify-between px-1">
          <button
            class="flex cursor-pointer items-center gap-1 rounded-lg border px-4 py-2 text-xs
              font-bold transition-all disabled:cursor-not-allowed disabled:opacity-40"
            :class="
              isDarkMode
            ...
            @click="currentPage--"
          >
            <ChevronLeft class="h-4 w-4" /> {{ $t("pet.history.prevPage") }}
          </button>

          <span
            class="font-mono text-xs font-bold tracking-widest transition-colors"
            :class="isDarkMode ? 'text-slate-400' : 'text-slate-500'"
          >
