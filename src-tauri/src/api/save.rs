...
pub async fn load_save(app: AppHandle, save_id: i32) -> Result<WebInitData, String> {
    ...
    let snapshot: GameStatusSnapshot = serde_json::from_str(&save_model.status).unwrap_or_default();
    {
        ...
        let possessed_before = gs.possessed_role_id;
        gs.apply_snapshot(&snapshot, db).await;
        // 快照若把附身实体同时记成当前对话对象，就读出了"自己跟自己说话"的死锁态：
        // 优先让在场的另一个 AI 接管话筒，没有候选才退回默认身份并刷新玩家缓存。
        // apply_snapshot 内部已刷新 human_role_ids，这里可直接用。
        if gs.possessed_role_id != PLAYER_ROLE_ID
            && gs.current_role_id == Some(gs.possessed_role_id)
        {
            let takeover = gs
                .present_role_ids
                .iter()
                .copied()
                .find(|id| *id != gs.possessed_role_id && !gs.human_role_ids.contains(id));
            match takeover {
                Some(id) => {
                    tracing::warn!(
                        "读档收敛：附身实体与对话对象重合，话筒移交 role_id={}",
                        id
                    );
                    gs.current_role_id = Some(id);
                }
                None => {
                    tracing::warn!(
                        "读档收敛：附身实体与对话对象重合且无可接管 AI，回退默认身份 possessed_role_id={}",
                        gs.possessed_role_id
                    );
                    gs.possessed_role_id = PLAYER_ROLE_ID;
                    if let Err(e) = gs.refresh_possessed_cache(db).await {
                        tracing::warn!("读档收敛后刷新附身缓存失败: {}", e);
                    }
                }
            }
        }
